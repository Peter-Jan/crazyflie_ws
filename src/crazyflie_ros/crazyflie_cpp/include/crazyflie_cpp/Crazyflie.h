...
public:
  LogBlockGeneric(
    ...
    void* userData,
    std::function<void(uint32_t, std::vector<double>*, void* userData,const std::vector<std::string>&)>& callback)
    : m_cf(cf)
    , m_userData(userData)
    , m_callback(callback)
    , m_id(0)
...
private:
  void handleData(crtpLogDataResponse* response, uint8_t size) {
    ...
    uint32_t time_in_ms = ((uint32_t)response->timestampHi << 8) | (response->timestampLo);
    m_callback(time_in_ms, &result, m_userData,varNames);
  }

private:
  Crazyflie* m_cf;
  void* m_userData;
  std::function<void(uint32_t, std::vector<double>*, void*,const std::vector<std::string>&)> m_callback;
  uint8_t m_id;
