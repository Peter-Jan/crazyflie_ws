...
private:
  ...
  void run()
  {
    ...
    if (m_enableLogging) {
      ...
      for (auto& logBlock : m_logBlocks)
      {
        ROS_INFO("log.var: %s",&logBlock.variables[0][0]);
        std::function<void(uint32_t, std::vector<double>*, void* userData,const std::vector<std::string>&)> cb =
          std::bind(
  ...
  void onLogCustom(uint32_t time_in_ms, std::vector<double>* values, void* userData,const std::vector<std::string>& fileName) {
    ...
    std::ofstream myfile;
    std::string varFile;
    std::vector<std::string> fileCpy;
    fileCpy = fileName;
    int i = 0;
    for (auto&& var : fileName)
    {
      auto pos = var.find(".");
      std::string first = var.substr(0, pos);
      std::string second = var.substr(pos+1);
      varFile = first + "_" + second + ".txt";
      myfile.open (varFile,std::ios_base::app);
      myfile << (double)values[0][i];
      myfile << ", ";
      myfile.close();
      ++i;
    }
