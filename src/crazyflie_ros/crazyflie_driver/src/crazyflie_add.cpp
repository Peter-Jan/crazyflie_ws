...
int main(int argc, char **argv)
{
  ...
  n.getParam("enable_logging_battery", enable_logging_battery);
  ...
  n.getParam("genericLogTopics", genericLogTopics);
  std::vector<int> genericLogTopicFrequencies;
  n.getParam("genericLogTopicFrequencies", genericLogTopicFrequencies);
  
  // ROS_INFO("sizeTopic: %lu",genericLogTopics.size());
  // ROS_INFO("sizeFreq: %lu",genericLogTopicFrequencies.size());

  // std::vector<std::string> myVar;
  // genericLogTopics.push_back("stabilizer");
  // genericLogTopicFrequencies.push_back(10);
  // myVar.push_back("pitch");

  ROS_INFO("logSize: %lu ",genericLogTopics.size());
  ROS_INFO("freqSize: %lu ",genericLogTopicFrequencies.size());
